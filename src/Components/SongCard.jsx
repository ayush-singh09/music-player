...
function SongCard({ data }) {
  const { name, creator, path, photo } = data;
  const { setAdd, queue, setQueue } = useContext(myContext);
  ...
  return (
    <div className="h-72 w-60 bg-black rounded-md p-3 flex flex-col justify-between leading-none">
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={() =>
            setAdd({ name: name, creator: creator, photo: photo, path: path })
          }
        ...
        <button
          onClick={() => {
            setQueue(() => {
              return [
                { name: name, creator: creator, photo: photo, path: path },
              ];
            });
          }}
          className="text-zinc-800 font-semibold text-sm bg-green-500 h-7 w-7 flex justify-center items-center rounded-full"
        >
          <MdPlayArrow />
        </button>
      </div>
