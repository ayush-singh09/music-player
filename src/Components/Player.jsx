  ...
  if (queue.length === 0)
    ...
    return (
      <div className={`absolute transition-all ease-in-out duration-500 text-white ${isFull?"justify-end h-full flex-col gap-6 pb-10":"justify-between h-20"} flex items-center px-5 w-full bg-gradient-to-br from-black via-zinc-900 to-black bottom-0 left-0 border-t-2 border-white`}>
        ...
        <div className={`transition-all ease-in-out duration-500 ${isFull?"w-full px-10 flex-col":"items-center"} flex  gap-4  overflow-hidden`}>
          <div className={`transition-all ease-in-out duration-500 ${isFull?"h-64 w-64 rounded":"h-14 w-14 rounded-full"} bg-red-400  overflow-hidden`}>
        ...
        <div className={`transition-all ease-in-out duration-500 items-center gap-2 ${!isFull && window.innerWidth<1000?"hidden":"flex"} ${isFull?"w-full px-10":""}`}>
          <h1>
          ...
          <input
            ...
            className={`inp transition-all ease-in-out duration-500 h-1 ${isFull?"w-full":"w-64"} bg-zinc-500 cursor-pointer`}
          />
          <h1>
